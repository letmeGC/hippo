<?php 
	...
	$__defaultData = $__tplData->getDefaultData();
...
<form accept-charset="UTF-8" action="/Admin/mineupdate?tb=<?=$__tplTbName?>" class="stdform" enctype="multipart/form-data" method="post">
	<?php foreach ($__tplTbDesc as $key => $value): ?>
		<?php if ($value["IsUserData"] == true || $value["Extra"]=="auto_increment"): ?>
			<?php continue; ?>
        <?php elseif ($value["Field"]=='dealer_code'): ?>
            <?php continue; ?>
		<?php endif; ?>
		<p>
			...
			<span class="field">
				<?php if ($value["readonly"]==true): ?>
					<?php echo sprintf("<input class=\"normalinput\" id=\"data[%s]\" name=\"data[%s]\" size=\"30\" type=\"text\" value=\"%s\" readonly=\"readonly\">",$value["Field"],$value["Field"],$__defaultData[$value["Field"]]); ?>
				...
				<?php else: ?>
					<?php if ($value["FieldSelectType"]=="select"): ?>
						...
						<?php 
							foreach ($value["FieldSelectValue"] as $key => $value) {
							     echo sprintf("<option value=\"%s\"  >%s</option>",$key,$value);
							}
						?>
						</select>
					<?php elseif( $value["FieldSelectType"]=='img'): ?>
					<?php echo sprintf("<input  id=\"%s\" name=\"%s\" size=\"30\" type=\"file\" >",$value["Field"],$value["Field"]);  ?>
					<?php elseif( $value["FieldSelectType"]=='time'): ?>
						<?php echo sprintf("<input class=\"normalinput\" id=\"data[%s]\" name=\"data[%s]\" size=\"30\" type=\"text\"  onClick=\"WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm'})\" >",$value["Field"],$value["Field"]); ?>
					<?php elseif( $value["FieldSelectType"]=='textarea'): ?>
						<?php echo sprintf("<textarea  rows=\"7\" cols=\"20\" id=\"data[%s]\" name=\"data[%s]\" size=\"30\" ></textarea>",$value["Field"],$value["Field"]); ?>
					...
					<?php else: ?>
						<?php if(in_array($value["Field"],array('price_low','price_mid','price_high')) &&  $__tplTbName=='Product'): ?>
							...
							<?php echo sprintf("<input class=\"normalinput\" onblur=\"blurfunc('".$value["Field"]."','".$__tplTbData[$value["Field"]]."')\" id=\"%s\" name=\"data[%s]\" size=\"30\" type=\"text\" >",$value["Field"],$value["Field"]); ?>
								
						<?php else: ?>
							<?php echo sprintf("<input class=\"normalinput\" id=\"data[%s]\" name=\"data[%s]\" size=\"30\" type=\"text\" >",$value["Field"],$value["Field"]); ?>
                    	<?php endif; ?>
					<?php endif; ?>
				<?php endif; ?>	
			</span>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){  
        ...
        ue.ready(function() {//编辑器初始化完成再赋值  
            ue.setContent('<?=$__tplData->rich_text?>');  //赋值给UEditor  
        });  
  ...
  $(document).ready(function(){  
        ...
        ue.ready(function() {//编辑器初始化完成再赋值  
            ue.setContent('<?=$__tplData->spesification?>');  //赋值给UEditor  
        });  
  ...
  $(document).ready(function(){  
        ...
        ue.ready(function() {//编辑器初始化完成再赋值  
            ue.setContent('<?=$__tplData->introduction?>');  //赋值给UEditor  
        });  
  ...
  $(document).ready(function(){  
        ...
        ue.ready(function() {//编辑器初始化完成再赋值  
            ue.setContent('<?=$__tplData->warranty?>');  //赋值给UEditor  
        });  
  ...
  $(document).ready(function(){  
        ...
        ue.ready(function() {//编辑器初始化完成再赋值  
            ue.setContent('<?=$__tplData->related?>');  //赋值给UEditor  
        });  
